...
// ========================================
// 【重要】此為與 AI-PT 共用的正式 Apps Script 原始碼備份。
// 客戶確認信品牌已改為「AI 俱樂部」。
// 修改後請到 https://script.google.com 開啟「既有」專案，
// 覆蓋 Code.gs 後「部署 → 管理部署 → 編輯 → 新版本」，
// 保持同一個 Web App URL，不要新建專案或試算表。
//
// 部署說明：
// 1. 使用既有 Google Sheet（推廣人員 / 客戶報名記錄 / 評估地點）
// 2. 前往 https://script.google.com 開啟既有專案
// 3. 將此代碼覆蓋到 Code.gs
// 4. 確認下方 SPREADSHEET_ID 仍為正式 Sheet
// 5. 部署 → 管理部署 → 編輯 → 版本選「新版本」→ 部署
// 6. 確認 Web App URL 不變（與前端 GOOGLE_SCRIPT_URL 相同）
...
自動通知系統
    ...
    // ========================================
    if (customerEmail) {
      const customerSubject = `感謝您報名「AI 俱樂部」`;
